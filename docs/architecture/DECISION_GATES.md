...
Status: normative planning gates

The product plan fixes the v0.1 direction, but cross-cutting semantics are accepted only when the milestone needs them. Record each completed gate as one or more ADRs with executable acceptance cases.

An issue cannot quietly decide a gate while implementing dependent behavior.

## D0 — Repository and source policy
...
- Independent Blackglass UI implementation; no reference source reuse without separate license and attribution approval.
...
- Coding-agent identity may be disclosed in PR metadata; do not invent human authorship or add attribution trailers without an explicit owner policy.
...
Scope is organizational context, not an execution allowlist. Every representable action remains runnable. An outside-scope or unusually large action may show one warning, but `Continue` is always present and covers the full run. Adding targets to saved scope is optional. Shared/CDN destinations are recorded clearly rather than silently rewritten into saved scope.
...
- the runner never applies target, purpose, or risk-tier policy;
- executable and argv spawning only; no shell strings;
- an expired or superseded lease cannot append results;
...
Accepted v0.1 baseline: evidence is not encrypted at rest. Filesystem permissions and documented, rehearsed backup/restore are required.

Immutability prevents runs and parsers from rewriting artifacts. It does not prevent documented owner retention or deletion.
...
Loopback-only is the supported default until this gate is complete. LAN support is a feature, not just a bind-address change.
...
No arbitrary frontend code and no automatic plugin updates in v0.1.
...
- v0.1 does not give the model a hidden shell, filesystem, or database channel outside installed capabilities;
...
- content never sent to a model endpoint;
...
“OpenAI-compatible” means the tested compatibility profile, not every server that uses the label.

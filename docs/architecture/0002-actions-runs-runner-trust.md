...
Every new runner mutation supplies authenticated runner ID, pinned session ID, Run ID, lease ID, fence, and a positive per-lease sequence. After the idempotency lookup described above, a new mutation is accepted only if all values match the current unexpired lease. A superseded fence returns 409 `stale_fence`; an expired lease returns 409 `lease_expired`; another identity/session returns 403 `lease_owner_mismatch`. None append an event or terminal state. Exact stored replays return their original result without reapplying those now-stale lease checks.

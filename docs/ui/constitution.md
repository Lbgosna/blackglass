...
Status: normative v0.1 interaction baseline
...
The main workspace is one shared inset layout. It is a flex column with `min-width: 0`, its own background and static grain, and stable geometry across routes. Pages do not create incompatible bespoke shells.
...
- Collapse is full off-canvas, not an icon rail.
...
- The resize rail changes width only. It is not an implicit collapse control.
...
- Labels remain visible; tooltips do not replace them.
...
- Hidden, collapsed, or unrendered items do not participate in jump navigation or multi-selection.
- Long lists may use `content-visibility: auto` with an appropriate intrinsic size.

Blackglass maps this hierarchy to its own entities. Likely examples are active runs or current work as cards, queued/paused work in a shelf, and completed/reviewed work as compact history. Domain language is determined by Blackglass contracts, not by a generic list component.
...
- Capability-gated actions that cannot execute are not rendered.
...
Blackglass warnings inform the operator without becoming approval workflows.
...
- `Add to scope & run` is available when saved scope is relevant but is never required.
...
- After `Continue`, no second confirmation or approval card appears for the same action.
- Scope mismatch never disables a Run button.
- The dialog places initial focus predictably, supports Enter to continue and Escape to cancel, and announces the warning without trapping screen-reader users in verbose target data.
- Timeline and run details record that the warning was continued, but warning history does not shame, score, or block the operator.
...
- Active resize: no transition; updates are animation-frame bounded.
...
With `prefers-reduced-motion`, disable list auto-animation, working/status animation, shimmer, and nonessential transforms or fades. Geometry changes may become immediate. Reduced-motion behavior is enforced by shared policy, not remembered separately by each component.

Continuously repainting decorative effects are not allowed.

## 8. Theme and tokens

Colors are semantic roles. Components do not contain raw presentation colors.
...
- Grain is a static texture on its owning surface, never a fixed fullscreen overlay.
...
- Mobile layouts are purpose-built sheets, not compressed desktop sidebars.
...
- Every action available on hover is also available by focus and touch.
...
- A required composite row supports Enter and Space, and nested actions do not trigger the row action.
- Inline rename focuses and selects on entry; Enter commits, Escape cancels, and blur commits.
- Status always has text and never relies on color alone.
- Ticking durations are excluded from live regions.
- Stable working, completed, and error labels may use `role="status"`; per-second updates may not.
...
- Background errors do not destroy still-valid stale data.
...
- Error text is safe for display and does not expose secrets, raw command flags, or untrusted markup.

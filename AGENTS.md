...
Blackglass is a fast operator tool for CTFs, labs, and assessments. It does not police the operator.

- Scope organizes targets, filters, evidence, and reports. It is not an authorization boundary.
...
- The v0.1 model uses typed capabilities rather than arbitrary shell, filesystem, or database access; this is an implementation boundary, not target censorship.
